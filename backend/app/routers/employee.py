...
@employee_router.patch("/wishes-points/", summary='Изменяет "любимые точки" сотрудника(определяет по токену)')
def change_my_wishes_points(
    points: List[str] = Body(..., embed=True),
    user: dict = Depends(get_current_employee)
):
    if not points:
        change_point_wishes(user['name'], POINTS, 'remove')
    else:
        change_point_wishes(user['name'], points, 'set')
    return {'message': 'Points changed successfully'}


@employee_router.patch("/wishes-days/", summary='Изменяет "любимые дни" сотрудника(определяет по токену)')
def change_my_wishes_days(
    days: List[str] = Body(..., embed=True),
    user: dict = Depends(get_current_employee)
):
    if not days:
    ...
    return {'message': 'Days changed successfully'}
